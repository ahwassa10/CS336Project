...
{
	String fare = ("SELECT ticket_id, SUM(flight_fare) AS total_fare FROM Travel.parts GROUP BY ticket_id HAVING total_fare = '" + filter1 +"';");
	ResultSet resultFare = stmt3.executeQuery(fare);
	
	while(resultFare.next())
	{	
	str += (" AND ticket_id = " + resultFare.getString("ticket_id"));
	}
...
if(!filter3.isBlank())
{
	String airline = ("SELECT p.ticket_id FROM Travel.parts p WHERE airline_id = '" + filter3 + "';");
	ResultSet resultAirline = stmt.executeQuery(airline);
	while(resultAirline.next())
	{
	String ticketAirlineID = resultAirline.getString("ticket_id");
	str += (" AND ticket_id = '" + ticketAirlineID + "'");
	}
}
...
while (result.next()) {
	...
	String airlineIDs = ("SELECT airline_id FROM Travel.parts WHERE ticket_id = '" + result.getString("ticket_id") + "';");
	ResultSet airlineIDS2 = stmt2.executeQuery(airlineIDs);
	while(airlineIDS2.next())
	{
	out.print(airlineIDS2.getString("airline_id"));
	}
	out.print("</td>");
	out.print("<td>");
	...
	String fares = ("SELECT ticket_id, SUM(flight_fare) AS total_fare FROM Travel.parts GROUP BY ticket_id HAVING ticket_id = '"+ result.getString("ticket_id") + "';");
	ResultSet fares2 = stmt3.executeQuery(fares);
	while(fares2.next())
	{
	out.print(fares2.getString("total_fare"));
	}
